		...
		for(int y = 0; y < data.height; ++y)
		{
			for(int x = 0; x < data.width; ++x)
			{
				...
				if(tile_id > 0)
				{
					...
					if (strcmp(layer->name.GetString(), "fondo") == 0)
					{
						App->render->Blit(tileset->texture, pos.x, pos.y, &r, SDL_FLIP_NONE, 0.15f);
					}
					if (strcmp(layer->name.GetString(), "forgound") == 0)
					{
						App->render->Blit(tileset->texture, pos.x, pos.y, &r, SDL_FLIP_NONE, 0.5f);
					}
					if (strcmp(layer->name.GetString(), "arboles 2") == 0)
					{
						App->render->Blit(tileset->texture, pos.x, pos.y, &r, SDL_FLIP_NONE, 0.25f);
					}
					else {
						App->render->Blit(tileset->texture, pos.x, pos.y, &r);
					}
...
bool j1Map::Load(const char* file_name)
{
	...
	for (object = map_file.child("map").child("objectgroup"); object && ret; object = object.next_sibling("objectgroup"))
	{
		...
		if (object_name2 == "colliders" || object_name2 == "pinchos" || object_name2 == "plataformas" || object_name2 == "win") {
